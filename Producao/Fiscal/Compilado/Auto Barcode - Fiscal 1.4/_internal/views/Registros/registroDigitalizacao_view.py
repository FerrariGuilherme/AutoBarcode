...
class DigitalizacaoView:
    ...
    def view(self,page:ft.page,params:Params,basket:Basket):
        ...
        def abrir_remessa(e):
            ...
            if len(bd.consultar_database(f"SELECT * FROM 'processos.remessa' WHERE remessa = '{remessa}' and tipo = 'DIGITALIZACAO' and comarca = '{comarca}'")) >= 1:
                row_para_exibir_msg_de_erro_remessa.controls=[]
            ...
            if comarca != None and destino != None and data != None and remessa != None:
                ...
                bd.alterar_database(f"INSERT INTO 'processos.remessa' (remessa, tipo, comarca, responsavel, data) VALUES ('{remessa}', 'DIGITALIZACAO', '{comarca}', '{responsavel}', '{data}')")
        ...
        def salvarRemessa(e):
            ...
            def registra_bd(e):
                ...
                bd.alterar_database(f'UPDATE "processos.remessa" SET processos = "{listagem_processos}" WHERE remessa = "{txtfield_remessa.value}" AND tipo = "DIGITALIZACAO" and comarca = "{comarca}"')
                txtfield_scanner.disabled=True
                #Adiciona os processos nas demais tabelas
                for processo in listagem_processos:
                    #Atualiza a tabela pendente de saida
                    bd.alterar_database(f"UPDATE 'processos.pendente_de_saida' SET situacao_atual = 'DIGITALIZACAO' WHERE processo = '{processo[0]}'")
                    #Adiciona os processo da remessa na tabela processo.registro
                    bd.alterar_database(f"INSERT INTO 'processos.registro' (processo, registro, data, remessa, providencia, situacao, responsavel, observacao, comarca) VALUES ('{processo[0]}', 'DIGITALIZACAO', '{txtfield_data.value}', '{txtfield_remessa.value}', '{processo[3]}', '{processo[4]}', '{usuario[0][1]}', '', '{comarca}')")
...
Tipo: DIGITALIZACAO  -  {usuario[0][1]}

Processo encaminhado para digitalização.
{drop_destino.value}
...
"""
        ...
        def imprimir(e):
            ...
            criar_pdf(nome_arquivo=f"{aponta_pdf}RD_{txtfield_remessa.value}_{comarca}.pdf", 
            ...
            filename = f"{aponta_pdf}RD_{txtfield_remessa.value}_{comarca}.pdf"
        ...
        def geraNumeroRemessa():
            while True:
                ...
                if len(bd.consultar_database(f"SELECT remessa FROM 'processos.remessa' WHERE tipo = 'DIGITALIZACAO' and remessa = '{remessa}'")) != 0:
                    pass
                else:
                    return remessa
                    break
        ...
        drop_destino = ft.Dropdown(width=430, height=55, text_size=15, border_width=0.8, border_color=ft.colors.BLACK, disabled=False)
        #Coleta as opcoes de destino do banco de dados
        for item in bd.consultar_database("SELECT destino FROM 'sistema.origem_destino' WHERE destino = 'Digitalização (INTERNO)' OR destino = 'Digitalização (EMPRESA)'"):
            if item[0] == None:
                break
            drop_destino.options.append(ft.dropdown.Option(f"{item[0]}"))
        
        txtfield_data = ft.TextField(label="Data de Digit.", width=186, prefix_icon=ft.icons.CALENDAR_MONTH_OUTLINED, hint_text="DD/MM/AAAA", prefix_text="  ", height=50, border_width=0.8, border_color=ft.colors.BLACK, value=data_atual())
        
        txtfield_remessa = ft.TextField(width=186, hint_text="Remessa de Entrada", height=50, border_width=0.8, border_color=ft.colors.GREEN, read_only=True, value=f"{geraNumeroRemessa()}", prefix_text="RD - ")
                ...
                ft.Column(
                    ...
                    controls=[
                        ...
                        ft.Row(
                            ...
                            controls=[
                                ft.Text(value="Digitalização", weight=ft.FontWeight.BOLD)
                            ]
                            ),
                        #DIVISORIA
                        ft.Divider(height=1),
